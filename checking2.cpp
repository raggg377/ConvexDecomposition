...
void Decompose()
{
    ...
    while (n > 3 and ctr--)
    {
        ...
        if (L.back() != v[2])
        {
            ...
            cout << L.back()->pos.x << " " << L.back()->pos.y << endl;
            // concave = d;
            cout << "make new dcel " << endl;
            decomp[poly_cnt - 1].print_dcel();
            // decomp.push_back(d);
            cout << "checking L.back" << endl;
            cout << L.back()->pos.x << " " << L.back()->pos.y << endl;
...
vector<lpv *> make_LPV(Vertex *v)
{
    ...
    for (int i = 0; i < decomp.size(); i++)
    {
        for (int j = 0; j < decomp[i].vertexList.size(); j++)
        {
            ...
            {
                Vertex *n_next = decomp[i].vertexList[j]->inc_edge->next->org;
                Vertex *n_prev = decomp[i].vertexList[j]->inc_edge->twin->next->next->org;

                Vertex *m_next = v->inc_edge->next->org;
                Vertex *m_prev = v->inc_edge->twin->next->next->org;
                if (n_next->pos == m_next->pos)
                {
                    LPVs.resize(count + 1);
                    LPVs[count] = new lpv(i, m_next);
                }
                if (n_prev->pos == m_prev->pos)
                {
                    LPVs.resize(count + 1);
                    LPVs[count] = new lpv(i, n_next);
                }
            }
        }
    }
...
void Merge()
{
    ...
    for (int j = 0; j < diag_cnt + 1; j++)
    {
        ...
        if ((LPVs.size() > 2 && LPVt.size() > 2) ||
            (LPVs.size() > 2 && isConvex(vt)) ||
            (LPVt.size() > 2 && isConvex(vs)) ||
            (isConvex(vs) && isConvex(vt)))
        {
        }
    }
}
